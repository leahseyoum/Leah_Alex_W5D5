...
def harrison_ford
  ...
  Movie.joins(:actors).where("name = 'Harrison Ford' AND ord > 1").select(:id, :title)
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group(:id).order('COUNT(castings.id) DESC').limit(3).select(:id, :title)
end
